...
function smallDropInit() {
  return (s) => {
    s.attr("class", "small-drop").each(function ({ levs }, i) {
      const s = d3.select(this);

      s.call(gradientInit(levs, i));
...
export function updateSmallDropSVG(
  ...
  const getStartLoc = ({ globalX, globalY }) =>
    `translate(${globalX}, ${globalY})`;
  const getEndLoc = ({ globalX, globalY, x, y }) =>
  ...
  container
    ...
    .data(waterdrops)
    .join((enter) => {
      return enter.append("g").call(smallDropInit());
    })
